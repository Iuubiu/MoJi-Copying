...
| 练笔时要做的事 | 工具里的做法 |
| --- | --- |
| 分屏对照、照着抄 | **双栏**（默认）：左原文、右抄写，两栏**同宽、同起点、逐行对齐**；写到最后一行立即整行下滚，底部常年空出一行。**单栏**：只留抄写栏，整章原文铺成灰底稿 —— 你写的字盖在它上面，抄过去变黑、抄错变红，**底稿本身钉住不动**（不随输入前后移动）—— 工具栏一键切换，选择记进设置 |
...
| 书架管理 | 卡片悬浮出「⋯」：改书名 / 作者 / 简介（导入识别不准时在这里改），或直接从书架删除；书架可在「按最近练习」与「按书名」之间切换排序，选择记在本地 |
